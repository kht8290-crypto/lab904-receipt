...
## [2026-06-10] — 영수증 사진 전체 자동입력 (비전)

### 추가
- 사진 등록 시 AI가 가능한 모든 필드 자동 입력:
  금액 · 날짜 · 상호명(용도) · 결제수단 · **카드(뒤 4자리로 등록 카드 매칭)** · 계정과목 · 증빙유형
- 못 읽은 항목은 "⚠️ 못 읽은 항목: ... — 직접 선택" 토스트로 안내
- 계정과목 목록을 클라이언트(앱 CATEGORIES)에서 Apps Script로 전송 → 항상 동기화
  (기존엔 Apps Script 하드코딩 목록이 앱과 달라 오분류)
- 계정과목 매칭에 NFC 정규화 적용

---

## [2026-06-10] — 영수증 사진 금액 자동인식 (Claude Haiku 비전)

### 추가
- 사진 등록 시 Claude Haiku 비전이 영수증에서 총 결제 금액 자동 추출 → 금액 칸 자동 입력
- Apps Script `action: extractReceipt` (이미지 base64 수신 → 비전 호출)
- 전송 전 클라이언트에서 이미지 1200px로 축소(JPEG) → 용량/비용 절감
- 못 읽으면 토스트로 직접 입력 안내

### 비고
- AI 계정과목 분류를 버튼 방식으로 전환(타이핑 자동분류 제거, 실패 시 직접선택 패널 펼침)
