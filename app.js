...
function renderSettleData(){
  ...
  // 증빙 미입력은 분기/연도와 무관한 데이터 품질 경고 → 전체 범위로 집계(viewer 필터와 동일 기준)
  const noVoucher=receipts.filter(isVoucherMissing).length;
...
function seedIfEmpty(){
  // 실제 운영 중 — 데모 시드 비활성화(빈 설치 때 데모가 주입돼 Drive로 동기화되며 실제 데이터를 오염시키는 문제 방지)
  return;
...
function renderViewer() {
  ...
  // 미입력 필터 활성 시 배너 카운트 동기화(영수증 수정으로 미입력이 줄면 즉시 반영)
  if (vFilters.noVoucher) {
    const allBtn = document.getElementById('vp-all');
    if (allBtn) allBtn.textContent = '❌ 미입력 ' + receipts.filter(isVoucherMissing).length + '건';
  }
